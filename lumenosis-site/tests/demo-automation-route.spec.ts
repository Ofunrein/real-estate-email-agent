...
test("automation generation requires bearer secret and immutable idempotency key", async()=>{ const s=await source(); expect(s).toContain("LUMENOSIS_DEMO_AUTOMATION_TOKEN"); expect(s).toContain("idempotencyKey"); expect(s).toContain("createHash"); expect(s).toContain("WHERE d.id = ?"); });

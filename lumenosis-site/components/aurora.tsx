...
export default function Aurora({
  colorStops = ['#cb6ce6', '#9333ea', '#cb6ce6'],
  amplitude = 1.0,
  blend = 0.5,
  speed = 0.4,
  className,
  ...
  useEffect(() => {
    ...
    const toFloatArr = (stops: string[]) => stops.map(h => { const c = new Color(h); return [c.r, c.g, c.b]; });
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorStops: { value: toFloatArr(colorStops) },
    ...
    const update = (t: number) => {
      ...
      if (program) {
        program.uniforms.uTime.value = t * 0.01 * (p.speed ?? 0.4) * 0.1;
        program.uniforms.uAmplitude.value = p.amplitude ?? 1.0;
        program.uniforms.uBlend.value = p.blend ?? 0.5;
        program.uniforms.uColorStops.value = toFloatArr(p.colorStops ?? colorStops);

...
const SETTINGS_CONNECTIONS: SettingsConnection[] = [
  { id: "email", channel: "email", label: "Gmail / Outlook", icon: Mail, accent: CHANNEL_ACCENT.email, detail: "Connect mailbox OAuth and sync connected calendars separately.", urls: [{ label: "Gmail OAuth", href: "/api/settings/email-account/connect" }, { label: "Composio Gmail", href: "/api/settings/email-account/composio-connect" }] },
  ...
  { id: "instagram", channel: "instagram", label: "Instagram DMs", icon: InstagramGlyph, accent: CHANNEL_ACCENT.instagram, detail: "Meta direct account with message send permissions.", urls: [{ label: "Connect Instagram", href: "/api/channels/meta/connect?channel=instagram&use_sdk=1" }, { label: "Composio fallback", href: "/api/settings/composio/connect/instagram" }] },
  { id: "messenger", channel: "messenger", label: "Messenger", icon: MessengerGlyph, accent: CHANNEL_ACCENT.messenger, detail: "Facebook Page messaging with direct Meta or fallback.", urls: [{ label: "Connect Messenger", href: "/api/channels/meta/connect?channel=messenger&use_sdk=1" }, { label: "Composio fallback", href: "/api/settings/composio/connect/facebook" }] },
  { id: "whatsapp", channel: "whatsapp", label: "WhatsApp", icon: WhatsAppGlyph, accent: CHANNEL_ACCENT.whatsapp, detail: "Meta Cloud WhatsApp number or Composio account.", urls: [{ label: "Connect WhatsApp", href: "/api/settings/composio/connect/whatsapp" }] },
...
function TakeoverHeaderControl({ thread, contextOpen, onToggleContext }: { thread: UnifiedThread; contextOpen: boolean; onToggleContext: () => void }) {
  ...
  React.useEffect(() => { const controller = new AbortController(); setTakenOver(false); fetch(`/api/threads/${encodeURIComponent(thread.id)}/takeover`, { signal: controller.signal }).then((res) => res.json()).then((data) => setTakenOver(Boolean(data?.isActive))).catch(() => undefined); return () => controller.abort(); }, [thread.id]);
  React.useEffect(() => {
    const onState = (event: Event) => {
      const detail = (event as CustomEvent<{ threadId?: string; active?: boolean }>).detail;
      if (detail?.threadId === thread.id) setTakenOver(Boolean(detail.active));
    };
  ...
  const setState = async (action: "take" | "release") => {
    ...
    try {
      ...
      setTakenOver(active);
      window.dispatchEvent(new CustomEvent("iris-takeover-state", { detail: { threadId: thread.id, active } }));
    } finally {
      setLoading(false);
    }
  };
...
function ManualReplyComposer({ thread }: { thread: UnifiedThread }) {
  ...
  const settingsKey = inboxSettingsKey(thread.channel);
  const channelOn = settingsKey ? model.inboxSettings.channels_enabled[settingsKey] !== false : true;
  const canSend = channelOn && thread.channel !== "website" && Boolean(thread.sendTo || thread.channel === "email");
  const callTarget = /\+?\d[\d\s().-]{7,}/.test(thread.sendTo || thread.contact) ? (thread.sendTo || thread.contact) : "";
  const canCall = Boolean(callTarget);
  ...
  React.useEffect(() => { const controller = new AbortController(); setTakenOver(false); setStatus(""); fetch(`/api/threads/${encodeURIComponent(thread.id)}/takeover`, { signal: controller.signal }).then((res) => res.json()).then((data) => setTakenOver(Boolean(data?.isActive))).catch(() => undefined); return () => controller.abort(); }, [thread.id]);
  React.useEffect(() => {
    const onState = (event: Event) => {
      const detail = (event as CustomEvent<{ threadId?: string; active?: boolean }>).detail;
      if (detail?.threadId === thread.id) setTakenOver(Boolean(detail.active));
...
function SettingsPanel({ model }: { model: ReturnType<typeof useInboxModel> }) {
  ...
  const save = async () => {
    ...
    setMessage("");
    try {
      const res = await fetch("/api/settings/inbox", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ settings: { draft_first: draftFirst, auto_send: autoSend, channels_enabled: channelsEnabled } }) });
      const data = await res.json().catch(() => ({}));
...
function AppointmentsPanel({ events, metrics }: { events: ActivityEvent[]; metrics: { appointments: number; liveTransfers: number } }) { const appointmentEvents = events.filter((event) => /appoint|book|showing|tour|transfer/i.test(`${event.intent} ${event.body}`)); return <section className="iris-wide-panel"><PanelTitle icon={CalendarDays} title="Appointments & transfers" subtitle="Actual booked outcomes, not speculative intent" /><div className="iris-mini-metrics"><div><span>Booked appointments</span><b>{metrics.appointments}</b></div><div><span>Qualified transfers</span><b>{metrics.liveTransfers}</b></div></div><ActivityTable events={appointmentEvents} empty="No booked appointment or transfer events loaded." /></section>; }
...
function channelCapabilities(thread: UnifiedThread, canSend: boolean, canCall: boolean) {

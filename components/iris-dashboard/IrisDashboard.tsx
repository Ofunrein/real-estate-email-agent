...
import { messageBubbleClassName, messageDisplayText, messagePreviewText } from "@/lib/messageDisplay";
...
function SettingsPanel({ model }: { model: ReturnType<typeof useInboxModel> }) {
  ...
  const cloneInputRef = React.useRef<HTMLInputElement>(null);
  const selectedVoice = voiceProfiles.find((voice) => voice.id === selectedVoiceId) || VOICE_PRESETS[0];
  // Sender + subject only: the organization preview never needs message bodies.
  const organizationThreads = React.useMemo(() => model.emailThreads.map((thread) => ({
    id: thread.id,
    from: thread.contact,
    subject: thread.messages[thread.messages.length - 1]?.subject || thread.preview,
  })), [model.emailThreads]);
...
function sanitizeEmailHtml(raw = "") { if (!raw) return ""; if (typeof window === "undefined") return ""; try { const DOMPurify = require("dompurify"); return DOMPurify.sanitize(raw, { USE_PROFILES: { html: true }, FORBID_TAGS: ["script", "iframe", "object", "embed"], FORBID_ATTR: ["onerror", "onload", "onclick", "onmouseover"] }); } catch { return ""; } }
...
// One-line rows (thread cards, activity feed) collapse on purpose. A message bubble must NOT:
// use messageDisplayText there so the stored paragraph structure survives to the screen.
function clampText(value = "", fallback = "No message yet") { return messagePreviewText(value, fallback); }

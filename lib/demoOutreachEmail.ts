/**
 * Outreach email rendering for demo outreach sends.
 *
 * Byte-identical port of lumenosis-site lib/email-html.ts. It is duplicated rather than
 * imported because the two repos do not share a package, and it is frozen by
 * tests/ts/demoOutreachEmail.test.ts against the recorded vectors so the two copies
 * cannot drift. This matters because after cutover this app performs the send: a
 * prospect must receive the same email bytes they would have received before.
 */
